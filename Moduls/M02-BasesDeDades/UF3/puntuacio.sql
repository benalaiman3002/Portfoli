...
if(new.lletra <> old.lletra) then 
	update prova_puntuacio_seg set puntuacion = (select count(*) from prova_puntuacio where lletra = old.lletra) where letra = old.lletra;
    update prova_puntuacio_seg set puntuacion = (select count(*) from prova_puntuacio where letra = new.lletra) where letra = new.lletra;
end if
DELIMITER ;
